<script lang="ts">
	...
	import type { CaseStudyView, EContentView, masterInputView, masterStatus } from '$lib/types/modules/research/research-types';
	...
	export let actionData: masterInputView;
	...
	async function handleDelete() {
		...
		const masterActions = json as { status : number, message : string}
		if (error) {
            ...
            if (masterActions.status == 200) {
			toast.success('Deleted Successfully !');
			let url: URL = new URL(`${PUBLIC_API_BASE_URL}/master-input-data-paginate`);
			paginateUrl.set(url);
		} else {
			toast.error(masterActions.message);
		}
	...
	{#if $showMenu}
		...
		>
			<div class="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
				<a
					href="{PUBLIC_BASE_URL}master-input-data/view/{actionData.id}"
				...
				>
				<a
					href="{PUBLIC_BASE_URL}master-input-data/edit/{actionData.id}"
					class="block w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-100 flex gap-2 items-center"
