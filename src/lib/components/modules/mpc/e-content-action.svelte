<script lang="ts">
	...
	import type { CaseStudyView, EContentView } from '$lib/types/modules/research/research-types';
	...
	export let actionData: EContentView;
	...
	let eContentId: number;

	const openModal = async () => {
		eContentId = actionData.id;
	...
	async function handleDelete() {
		console.log('delete', eContentId);
		isOpen.set(false);
		const { error, json } = await fetchApi({
			url: `${PUBLIC_API_BASE_URL}/delete-e-content?id=${eContentId}`,
			method: 'GET'
		});

		const eContentActions = json as {status : number , message : string}
		if (error) {
		...
		if (eContentActions.status == 200) {
			toast.success('Deleted Successfully !');
			let url: URL = new URL(`${PUBLIC_API_BASE_URL}/e-content-paginate`);
			paginateUrl.set(url);
		} else {
			toast.error(eContentActions.message);
		}
	}
</script>
	...
	{#if $showMenu}
		...
		>
			<div class="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
				<a
					href="{PUBLIC_BASE_URL}e-content/view/{actionData.id}"
				...
				<a
					href="{PUBLIC_BASE_URL}e-content/edit/{actionData.id}"
					class="block w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-100 flex gap-2 items-center"
