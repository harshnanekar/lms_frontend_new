<script lang="ts">
	...
	import type { BookPublicationRender, ChronicleActionView } from '$lib/types/modules/research/research-types';
	...
	export let actionData: ChronicleActionView;
	...
	let chronicleId: number;

	const openModal = async () => {
		chronicleId = actionData.id;
	...
	async function handleDelete() {
		console.log('delete button clicked', chronicleId);
		isOpen.set(false);

        const { error, json }= await fetchApi({
			url: `${PUBLIC_API_BASE_URL}/chronicle-delete?id=${chronicleId}`,
			method: 'GET'
		});

		const ChronicleAction = json as { status : number, message : string}
		...
		let url: URL = new URL(`${PUBLIC_API_BASE_URL}/chronicle-list-paginate?id=7`);
		paginateUrl.set(url);

		}else{
			toast.error(ChronicleAction.message);
		}
	...
	{#if $showMenu}
		...
		>
			<div class="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
				<a
					href="{PUBLIC_BASE_URL}meeting-chronicle/view/"
				...
				>
				<a
					href="{PUBLIC_BASE_URL}meeting-chronicle/edit/"
					class="block w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-100 flex gap-2 items-center"
