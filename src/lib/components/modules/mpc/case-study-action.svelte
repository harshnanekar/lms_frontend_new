<script lang="ts">
	...
	export let actionData: CaseStudyView;
	...
	async function handleDelete() {
		...
		const caseStudyActions = json as {status : number, message : string}
		if (error) {
		...
		if (caseStudyActions.status == 200) {
			toast.success('Deleted Successfully !');
			let url: URL = new URL(`${PUBLIC_API_BASE_URL}/case-study-paginate`);
			paginateUrl.set(url);
		} else {
			toast.error(caseStudyActions.message);
		}
	}
</script>
	...
	{#if $showMenu}
		...
		>
			<div class="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
				<a
					href="{PUBLIC_BASE_URL}case-study/view/{actionData.id}"
				...
				<a
					href="{PUBLIC_BASE_URL}case-study/edit/{actionData.id}"
					class="block w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-100 flex gap-2 items-center"
