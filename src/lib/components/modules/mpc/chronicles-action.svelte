<script lang="ts">
	...
	async function handleDelete() {
        ...
        const { error, json }= await fetchApi({
			url: `${PUBLIC_API_BASE_URL}/book-publication-delete?id=${bookPublicationId}`,
			method: 'GET'
		});

		const bookPublicationAction = json as { status : number, message : string}
		...
		if(bookPublicationAction.status == 200){
		...
		paginateUrl.set(url);
			...
			toast.error(bookPublicationAction.message);
		}
	...
	{#if $showMenu}
		...
		>
			<div class="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
				<a
					href="{PUBLIC_BASE_URL}book-publication/view/"
				...
				>
				<a
					href="{PUBLIC_BASE_URL}book-publication/edit/"
					class="block w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-100 flex gap-2 items-center"
