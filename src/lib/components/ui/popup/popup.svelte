<script lang="ts">
  ...
  <Modal bind:isOpen={$isOpen} size={modalwidthPercent} on:close={handleNoClick}>
	<div class="text-center p-8" slot="body">
	  <i class="fa-solid fa-trash text-primary text-[30px]"></i>
	  <h3 class="mb-5 lg:text-lg md:text-md sm:text-sm font-normal font-semibold text-gray-500 dark:text-gray-400 mt-2">
		{@html $confirmStore.confirmText ?? 'Are you Sure?'}
		</h3>
	<div class="flex flex-row justify-center mt-4 gap-2">
	  <button class="lms-btn lms-primary-btn md:text-md sm:text-sm" on:click={handleYesClick}>Yes, I'm sure</button>
	  <button class="lms-btn lms-secondary-btn  md:text-md sm:text-sm" on:click={handleNoClick}>No, cancel</button>
	</div>	
	</div>
