<script lang="ts">
    ...
    startDate = data.chronicleData.length > 0 ? new Date(data.chronicleData[0].start_date) : null;
    $: startFormattedDate = startDate; 
    
    let endDate : Date | null = new Date() ;
    endDate = data.chronicleData.length > 0 ? new Date(data.chronicleData[0].end_date) : null;
