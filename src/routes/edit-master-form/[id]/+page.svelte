<script lang="ts">
	import SkeletonLoader from './../../../lib/components/ui/loaders/skeleton-loader.svelte';
	...
	onMount(async () => {
		masterFormStore.set(data.masterForm);
        meetingName = data.masterForm?.meetingName
        meetingDescription = data.masterForm?.linkPassword
        $masterFormStore.acadYear = {
            value: data.masterForm?.acadYear,
            label: data.masterForm?.acadYear
        }
		await tick();
		console.log('masterFormStore>>>>>>>>', $masterFormStore);
		isLoading = false;
...
{#if isLoading}
    <SkeletonLoader/>
{:else}
	<h2 class="text-heading-2.5 font-medium">Viewing Meeting Titled :  
        <b class="bg-base text-label-md md:text-body-2 mr-3 gap-x-4 rounded-3xl px-4 py-1 text-black">{$masterFormStore.meetingName}</b>
    </h2>
		...
		<div class="mt-6 grid grid-cols-1 gap-5 lg:grid-cols-2 lg:gap-6">
			...
			<div class="col-span-full flex flex-wrap items-center gap-2">
				<DatePicker
					...
					bind:selectedDateTime={meetingDate}
				...
				{#each $masterFormStore.meetingDate as date, i}
					{@const formattedDate = formatDateTimeShort(new Date(date))}
					<div
					...
					>
						...
						<button
							...
							on:click={() => {
								...
								$masterFormStore.meetingDate = $masterFormStore.meetingDate.filter(
									(_, idx) => i !== idx
								);
							}}
						>
	...
	<Card title="Campus Details">
		{#each $masterFormStore.meetingSubject as sub}
			<CampusDetailCard
				...
				bind:dataToPopulate={campusDataToPopulate}
			/>
		{/each}
		<div class="my-3"></div>
		<button
	...
	<div class="mt-6 flex w-full items-center justify-end gap-x-4">
		<button class="lms-btn lms-secondary-btn px-8 py-3" on:click={clearForm}>Clear Form</button>
		<button class="lms-btn lms-primary-btn px-12 py-3" on:click={handlePublishMeeting}
