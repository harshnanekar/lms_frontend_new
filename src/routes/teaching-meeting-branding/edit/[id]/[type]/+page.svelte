...
<script lang="ts">
	...
	let teachingItems: { id: number, type: any, description: string, link: string, file: File[],isChecked?: boolean }[] =  data.inputData.teaching_data ? data.inputData.teaching_data : [];
	let meetingItems: { id: number, type: any, description: string, link: string, file: File[],isChecked?: boolean }[] = data.inputData.meeting_data ? data.inputData.meeting_data : [];
	let brandingItems: { id: number, type: any, description: string, link: string, file: File[],isChecked?: boolean }[] = data.inputData.branding_data ? data.inputData.branding_data : [];
	...
	let teachingDropdown = data.inputData.teaching_inputs ? data.inputData.teaching_inputs.message : [];
	let meetingDropdown  = data.inputData.meeting_inputs ? data.inputData.meeting_inputs.message : [] ;
	let brandingDropdown = data.inputData.branding_inputs ? data.inputData.branding_inputs.message : [] ;
		...
		<table class="lms-table">
			...
			<tbody>
				...
				<tr>
					...
					<td>
                        <div>
                           {#if item.type!=null && item.description!='' && item.link!='' && !item.isChecked}  
                           <label class="text-gray-500">Click To Upload New File <input type="checkbox" bind:checked={item.isChecked} /></label>
                            {#if item.isChecked}
                                <input type="file" multiple on:change={(e) => updateTeachingItem(item.id, 'file', [...e?.target?.files])} />
                            {:else}
                                <button class="lms-primary-btn mt-2" on:click={downLoadFiles}><i class="fa-solid fa-download text-md"></button>
                            {/if}
                           {:else}
                           <input type="file" multiple on:change={(e) => updateTeachingItem(item.id, 'file', [...e?.target?.files])} />
                           {/if}
					...
					<td>
						<button class="lms-btn lms-primary-btn" on:click={() => removeTeachingRow(item.id)}
                            disabled={item.type!=null && item.description!='' && item.link!='' ? true : false}>
							<MinusIcon />
							Remove
						</button>
