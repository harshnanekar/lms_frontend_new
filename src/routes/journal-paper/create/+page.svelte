...
import { getAbdcIndexed , getPaperType , getSchool ,getCampus , getPolicyCadre,
  getAllAuthor,getNmimsAuthor,getOtherAuthor,getForeignAuthor,getStudentAuthor
} from '$lib/utils/select.helper';
import { validateWithZod } from '$lib/utils/validations';
import { journalPaper , type JournalPaperReq } from '$lib/schemas/modules/research/master-validations';
...
$: publicationFormattedDate = publicationDate ?? null;
...
async function handleSubmit(){
 ...
 const result : any =  validateWithZod(journalPaper,journalObject);
...
Object.keys(result.data).forEach(key => {
  if (Array.isArray(result.data[key])) {
    result.data[key].forEach(item => {
      if (key === 'supporting_documents' && item instanceof File) {
        formData.append(`journal_paper[supporting_documents]`, item);
      } else {
        formData.append(`journal_paper[${key}][]`, item);
      }
    });
  } else {
    formData.append(`journal_paper[${key}]`, result.data[key]);
  }
});
...
<div class="rounded-2xl border-[1px] border-[#E5E9F1] p-4 !pt-0 shadow-card sm:p-6">
  <div class="min-h-[50vh] overflow-auto scroll modal-content max-h-[70vh]"> <!-- Adjust max-height as needed -->
    ...
    <div class="grid grid-cols-3 gap-[40px] p-4">
      ...
      <Input type="number" {isRequired} placeholder="Scopus Site Score" bind:value ={obj.scopus_site_score}/>
    </div>
    <div class="grid grid-cols-3 gap-[40px] p-4">
    ...
    <div class="grid grid-cols-3 gap-[40px] p-4">
      ...
      <div class="ml-2">
        ...
        <div class="flex flex-row gap-[20px] mt-2.5">
          <div class="flex flex-row">
            <input type="radio" id="html" class="lms-input-radio w-4" name="radio-button-national" bind:group = {obj.journal_type} checked={obj.journal_type == 1} value = {1}>
            <span class="text-[#888888] text-sm">International</span>
          </div>
          <div class="flex flex-row">
            <input type="radio" id="html" class="lms-input-radio w-4" name="radio-button-national"  bind:group = {obj.journal_type} checked={obj.journal_type == 1} value={2}>
      ...
      <div class="ml-2">
        ...
        <div class="flex flex-row gap-[20px] mt-2">
          <div class="flex flex-row">
            <input type="radio" id="html" class="lms-input-radio w-4" name="radio-button-wos" bind:group={obj.wos_indexed} checked={obj.wos_indexed == true} value={true}>
            <span class="text-[#888888] text-sm">Yes</span>
          </div>
          <div class="flex flex-row">
            <input type="radio" id="html" class="lms-input-radio w-4" name="radio-button-wos" bind:group={obj.wos_indexed} checked={obj.wos_indexed == true} value={false}>
    ...
    <div class="grid grid-cols-3 gap-[40px] p-4">
      ...
      <div class="ml-2">
        ...
        <div class="flex flex-row gap-[20px] mt-2">
          <div class="flex flex-row">
            <input type="radio" id="html" class="lms-input-radio w-4" name="radio-button-ugc" bind:group={obj.ugc_indexed} checked={obj.ugc_indexed == true} value={true} >
            <span class="text-[#888888] text-sm">Yes</span>
          </div>
          <div class="flex flex-row">
            <input type="radio" id="html" class="lms-input-radio w-4" name="radio-button-ugc" bind:group={obj.ugc_indexed} checked={obj.ugc_indexed == true} value={false}>
            <span class="text-[#888888] text-sm">No</span>
          </div>
        </div>
      </div>
    ...
    <div class="grid grid-cols-3 gap-[40px] p-4">
      ...
      isMultiSelect = {true}
		  />
      <Input {isRequired} type="number" placeholder="No. Of Foreign Authors" bind:value ={obj.foreign_authors_count}/>  
      <DynamicSelect
      isRequired={false}
    ...
    <div class="grid grid-cols-3 gap-[40px] p-4">
      <Input {isRequired} type="number" placeholder="No. Of Student Authors" bind:value ={obj.student_authors_count}/>    
      <div class="ml-2">
        <label class="text-[#888888] text-sm">Scopus Site Indexed<span class="text-danger text-sm">*</span></label>
        <div class="flex flex-row gap-[20px] mt-2">
          <div class="flex flex-row">
            <input type="radio" id="html" class="lms-input-radio w-4" name="radio-button-scs" bind:group={obj.scs_indexed} checked={obj.scs_indexed == true} value={true}>
            <span class="text-[#888888] text-sm">Yes</span>
          </div>
          <div class="flex flex-row">
            <input type="radio" id="html" class="lms-input-radio w-4" name="radio-button-scs" bind:group={obj.scs_indexed} checked={obj.scs_indexed == true} value={false}>
            <span class="text-[#888888] text-sm">No</span>
          </div>
        </div>
      </div>
